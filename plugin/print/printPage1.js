(function($){
    ...
    function toHtml(target, rows){
      ...
      console.log(rows)
        if(rows[0].courtRoomName !=undefined&&rows[0].cbrName == undefined&&rows[0].courtRoomId !=""){
           for (let index = 0; index < rows.length; index++) {
               const element = rows[index];
               element.courtName = element.courtRoomName;
               element.detail = "--";
           }
        }else if(rows[0].courtRoomName!=""&&rows[0].courtRoomName!=undefined&&rows[0].cbrName!=undefined){
            for (let index = 0; index < rows.length; index++) {
                const element = rows[index];
                element.courtName = element.cbrName;
                element.detail = "--";
            }
        }
